...
class Book {
public:
    Book(ISBN isbn_code, string title, string author, Genre genre, string copyright_date);
    string isbn() const { return m_ISBN; }
    string title() const { return m_title; }
    string author() const { return m_author; }
    string copyright_date() const { return m_copyright_date; }
    bool checked() const { return m_checked_out; }
    // check if a book is in (true) or out (false) of the library
    void book_in() { m_checked_out = false; }
    void book_out() { m_checked_out = true; }
private:
    string m_ISBN;
    string m_title;
    string m_author;
...
Book::Book(ISBN isbn_code, string title, string author, Genre genre, string copyright_date)
:m_ISBN{isbn_code.isbn()}, m_title{title}, m_author{author},
m_genre{genre}, m_copyright_date{copyright_date}
{
}

bool operator==(const Book &ba, const Book &bb){
    if (ba.isbn() == bb.isbn()) return true;
    return false;
}
